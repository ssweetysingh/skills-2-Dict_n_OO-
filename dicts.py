...
def melon_at_price(price):

    melons_name = {'Honeydew': 2.50,
    'Cantaloupe': 2.50,
    'Watermelon': 2.95,
    'Musk': 3.25,
    'Crenshaw': 3.25,
    'Christmas': 14.25}
    melon_found =[]
    for key,val in melons_name.items():
        if price == val:
            melon_found.append(key)
    melon_found.sort()
    return melon_found
...
def translate_to_pirate_talk(phrase):
    ...
    translate_words_split = phrase.split()
    converted_phrase = []
    for i in translate_words_split:
        converted_phrase.append(translate_eng.get(i,i))
    return ' '.join(converted_phrase)
